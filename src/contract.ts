 ...
 */
export type SkillPanelErrorCode =
  | 'bad-request'
  | 'workspace-not-found'
  | 'command-error'
  | 'internal'
...
export type SkillPanelResult<T> =
  ...
  | { readonly ok: false; readonly error: SkillPanelError }
...
export function ok<T>(value: T): SkillPanelResult<T> {
  return { ok: true, value }
}

export function fail<T>(code: SkillPanelErrorCode, message: string, details: Readonly<Record<string, unknown>> = {}): SkillPanelResult<T> {
  const error: SkillPanelError = { code, message, details }
  return { ok: false, error }
}

/** Read a string field from an unknown payload, tolerating absent payloads. */
export function readString(payload: unknown, field: string): string | undefined {
